    ...
    async def snapshot(self, symbol: str) -> MarketSnapshot:
        ...
        try:
            ...
            if df is not None and not df.empty:
                last_row = df.iloc[-1]
                # Vnstock 4.x usually returns raw close price (e.g. 36000)
                price_val = float(last_row.get('close', 0))
                # Convert raw prices to thousands to match Entry
                last_price = price_val / 1000 if price_val > 1000 else price_val
                
                volume = float(last_row.get('volume', 0))
                if len(df) > 1:
                    prev_val = float(df.iloc[-2].get('close', price_val))
                    prev_price = prev_val / 1000 if prev_val > 1000 else prev_val
                    if prev_price > 0:
